<?php  
	...
	if($id!='')
	{
		...
		$where = "";
		$where = "id <> ? and id_list = ? and id_cat = ? and type = ? and hienthi > 0";
		$params = array($id,$row_detail['id_list'],$row_detail['id_cat'],$type);
	...
	$curPage = $get_page;
	if($id>0) $per_page = $optsetting['soluong_spk'];
	else $per_page = $optsetting['soluong_sp'];
	$startpoint = ($curPage * $per_page) - $per_page;
	$limit = " limit ".$startpoint.",".$per_page;
	$sql = "select photo, ten$lang as ten, tenkhongdauvi, tenkhongdauen, giamoi, gia, giakm, id, type from #_product where $where order by stt,id desc $limit";
