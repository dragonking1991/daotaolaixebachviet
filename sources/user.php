<?php
		...
		if($iduser)
		{
		    ...
		    if(isset($_POST['capnhatthongtin']))
		    {
		        ...
		        if($password)
		        {
		            ...
		            if(!isset($row['id'])) $func->transfer("Mật khẩu cũ không chính xác","", false);
	...
	function signup()
	{
		...
		if(isset($row['id']) && $row['id'] > 0) $func->transfer("Tên đăng nhập đã tồn tại", $config_base."account/dang-ky", false);

		/* Kiểm tra email đăng ký */
		$row = $d->rawQueryOne("select id from #_member where email = ? limit 0,1",array($email));
		if(isset($row['id']) && $row['id'] > 0) $func->transfer("Địa chỉ email đã tồn tại", $config_base."account/dang-ky", false);
	...
	function doimatkhau_user()
	{
		...
		if(!isset($row['id'])) $func->transfer("Tên đăng nhập và email không tồn tại", $config_base."account/quen-mat-khau", false);
