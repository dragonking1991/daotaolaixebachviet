...
<?php
    ...
    $logo = $d->rawQueryOne("select ten$lang, photo, link, hienthi from #_photo where type = ? and act = ? limit 0,1",array('logo','photo_static'));
    ...
    if(isset($_POST['submit-newsletter']))
    {
        ...
        if(($scoreCaptcha >= 0.5 && $actionCaptcha == 'Newsletter') || $testCaptcha == true)
        {
            ...
            if($d->insert('newsletter',$data)){
                $func->transfer("Gửi đăng ký nhận tin thành công",$config_base);
            }
            else{
                $func->transfer("Gửi đăng ký nhận tin thất bại. Vui lòng thử lại sau",$config_base, false);
            }
            ...
            if($emailer->getEmail('tennguoigui')){
                $strThongtin .= '- Họ tên: <span style="text-transform:capitalize">'.$emailer->getEmail('tennguoigui').'</span><br>';
            }
            if($emailer->getEmail('emailnguoigui')){
                $strThongtin .= '- Email: <a href="mailto:'.$emailer->getEmail('emailnguoigui').'" target="_blank">'.$emailer->getEmail('emailnguoigui').'</a><br>';
            }
            if($dienthoai){
