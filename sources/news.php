<?php  
	...
	{
		/* Lấy bài viết detail */
		$row_detail = $d->rawQueryOne("select id, luotxem, ngaytao, id_list, id_cat, id_item, id_sub, type, ten$lang as ten, tenkhongdauvi, tenkhongdauen, noidung$lang as noidung, photo, options from #_news where id = ? and type = ? and hienthi > 0 limit 0,1",array($id,$type));
	...
	{
		/* Lấy cấp 1 detail */
		$news_list = $d->rawQueryOne("select id, ten$lang as ten,noidung$lang as noidung, tenkhongdauvi, tenkhongdauen, type, photo, options from #_news_list where id = ? and type = ? limit 0,1",array($idl,$type));
	...
	{
		/* Lấy cấp 2 detail */
		$news_cat = $d->rawQueryOne("select id, id_list, ten$lang as ten,noidung$lang as noidung, tenkhongdauvi, tenkhongdauen, type, photo, options from #_news_cat where id = ? and type = ? limit 0,1",array($idc,$type));
	...
	{
		/* Lấy cấp 3 detail */
		$news_item = $d->rawQueryOne("select id, id_list, id_cat, ten$lang as ten,noidung$lang as noidung, tenkhongdauvi, tenkhongdauen, type, photo, options from #_news_item where id = ? and type = ? limit 0,1",array($idi,$type));
	...
	{
		/* Lấy cấp 4 */
		$news_sub = $d->rawQueryOne("select id, id_list, id_cat, id_item, ten$lang as ten,noidung$lang as noidung, tenkhongdauvi, tenkhongdauen, type, photo, options from #_news_sub where id = ? and type = ? limit 0,1",array($ids,$type));
	...
	$curPage = $get_page;
	if($id>0) $per_page = $optsetting['soluong_tink'];
