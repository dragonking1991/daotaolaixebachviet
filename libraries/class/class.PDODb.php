...
class PDODb
{
        ...
        private function buildInsert($tableName, $insertData, $operation)
        {
            ...
            $this->createlogs(@$_SESSION[$login_admin]['username'],'insert',$tableName,$this->pdo()->lastInsertId());
            $this->reset();
        ...
        {
            $this->fillTable($this->getTableName($tableName),$insertData);
        ...
        public function rawQuery($query, $params = null)
        {
            ...
            $this->query = str_replace('#_', $this->prefix, $query);
            if (is_array($params)) {
                $this->params = $params;
            }
            $stmt = $this->prepare();
            ...
            if(strpos($this->query,'delete')!==false)
            {
                ...
                $this->createlogs(@$_SESSION[$login_admin]['username'],'query',"tables",$queryLogs);
            }
            $this->reset();
            return $result;
        }
        ...
        function getAllSchema($table=null){
            if(!count($this->_schema)){
                ...
                $rdata = array();
                foreach($rs as $k=>$v){
                    $rdata[$v['TABLE_NAME']][$v['COLUMN_NAME']] = array("COLUMN_KEY"=>$v['COLUMN_KEY'],"DATA_TYPE"=>$v['DATA_TYPE'],"IS_NULLABLE"=>$v['IS_NULLABLE'],"COLUMN_DEFAULT"=>$v['COLUMN_DEFAULT']);
                }
                $this->_schema = $rdata;
            }
            if($table != null && $this->_schema != null)
        ...
        public function update($tableName, $tableData, $numRows = null)
        {
            ...
            $this->createlogs(@$_SESSION[$login_admin]['username'],'update',$tableName,$wherelogs);
            $this->reset();
            $this->rowCount      = $stmt->rowCount();
            return $status;
        }
        ...
        public function createlogs($user='Unnamed',$act,$com,$id)
        {
            ...
            // Kiá»ƒm tra náº¿u chÆ°a cĂ³ thÆ° má»¥c thĂ¬ táº¡o má»›i
            if(!is_dir($logsfile))
            {
                mkdir($logsfile,0777,true);
                chmod($logsfile,0777);
            }
            // Láº¥y thĂ´ng tin ip vĂ  ngĂ y thĂ¡ng
            $ip_user = $this->get_client_ip();
            $name_file = date('d-m-Y',time());
            // Má»Ÿ file log Ä‘á»ƒ ghi náº¿u ko cĂ³ thĂ¬ táº¡o má»›i
            ...
            fclose($file);
            // Kiá»ƒm tra vĂ  xĂ³a file log cÅ© quĂ¡ 30 ngĂ y
            //$name_file_old = date('d-m-Y',time()-2592000);
    ...
    function dd($s){echo '<pre>';print_r($s);echo '</pre>';}
?>