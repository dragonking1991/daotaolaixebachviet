<?php
	...
	function save_item()
	{
		...
		$row_opt = $d->rawQueryOne("select id, options2 from #_news where id = ? and type = ? limit 0,1",array($id_opt, $type));
		...
		if($data)
		{
			...
			$data['gia1'] = (isset($data['gia1']) && $data['gia1'] != '') ? str_replace(",","",$data['gia1']) : 0;
			$data['gia2'] = (isset($data['gia2']) && $data['gia2'] != '') ? str_replace(",","",$data['gia2']) : 0;
