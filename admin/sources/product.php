<?php
	...
	function save_list()
	{
		...
		if($id)
		{
			...
			{
				$file_name_icon = $func->uploadName($_FILES['icon']["name"]);
				if($icon = $func->uploadImage("icon", $config['product'][$type]['img_type_list'], UPLOAD_PRODUCT,$file_name_icon))
				{
					$data['icon'] = $icon;
					$row = $d->rawQueryOne("select id, icon from #_product_list where id = ? and type = ? limit 0,1",array($id,$type));
					if(isset($row['id']) && $row['id'] > 0) $func->delete_file(UPLOAD_PRODUCT.$row['icon']);
				}
			}
