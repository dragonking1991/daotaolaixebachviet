...
is_readable('elfinder/vendor/autoload.php') && require 'elfinder/vendor/autoload.php';
...
$opts = array(
	...
	'roots' => array(
		...
		array(
			'id'            => '1',
			'driver'        => 'Trash',
			'path'          => $path_elfinder.'.trash/',
			'tmbURL'        => dirname($_SERVER['PHP_SELF']) . '../' .$path_elfinder .'.trash/.tmb/',
			'winHashFix'    => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
...
// Create dir trash
if(!file_exists($path_elfinder.'.trash/'))
{
	mkdir($path_elfinder.'.trash/', 0777, true);
}
